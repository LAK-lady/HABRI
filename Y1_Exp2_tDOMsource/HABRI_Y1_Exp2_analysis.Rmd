...
```{r}
data<-read.csv(file="Y1_Exp2_allData.csv", header=TRUE)
```
...
c) adding pre (initial) measurements as new field...
...
png(file="Y1_Exp2_tDOMsource/Products/SI_Figures/SI_Figure2.png", width=4,height=4, units="in", res=1200)
...
```{r}
png(file="Y1_Exp2_tDOMsource/Products/SI_Figures/SI_Figure10.png", 
...
SI_fig3
ggsave(SI_fig3, file="Y1_Exp2_tDOMsource/Products/SI_Figures/SI_Figure3_color.png",device="png", width=6, height=6, units="in")
...
SI_fig8
ggsave(SI_fig8, file="Y1_Exp2_tDOMsource/Products/SI_Figures/SI_Figure8_color.png",device="png", width=5, height=5, units="in", dpi=1200)
...
write.csv(SI_Table11, file="Y1_Exp2_tDOMsource/Products/SI_Figures/SI_Table11.csv")
```
...
SHtest_Chl<- data2 %>% #create new data frame focused on response
  filter(Treatment=="control" | Treatment =="manure" | Treatment=="crop"|Treatment=="wetland") %>% #treatments selected
  select(Lake, tDOM_added, tDOM_source, Replicate, Chla_ugL, pre_Chl)

### write the model ###
Chl_ancova<- (lm(data=SHtest_Chl, 
               log10(Chla_ugL)~log10(pre_Chl)+tDOM_source)) 
...
SI_fig11a<-ggplot(data=SHtest_Chl, #plots the model
       aes(x=log10(pre_Chl), y=log10(Chla_ugL), color=tDOM_source)) +
...
SI_fig11a
ggsave(SI_fig11a, file="Y1_Exp2_tDOMsource/Products/SI_Figures/SI_Figure11a_color.png",device="png", width=5, height=5, units="in", dpi=1200)
...
SHtest_PC<- data2 %>% #create new data frame focused on response
  filter(Treatment=="control" | Treatment =="manure"|Treatment=="crop"|Treatment=="wetland") %>% #treatments selected
  select(Lake, tDOM_added, tDOM_source, Replicate, PC_ugL, pre_PC)

### write the model ###
PC_ancova<- (lm(data=SHtest_PC, 
               log10(PC_ugL)~log10(pre_PC)+tDOM_source)) 
...
SI_fig12a<-ggplot(data=SHtest_PC, #plots the model
       aes(x=log10(pre_PC), y=log10(PC_ugL), color=tDOM_source)) +
  geom_point(aes(shape=Lake)) + 
  geom_smooth(method="lm", se=TRUE) +
  #scale_color_manual(values=c("skyblue3","saddlebrown")) +
  theme_classic() + #classic publication format
  labs(x="Pre Phycocyanin (ug/L), log10-transformed",
       ...
       color="Treatment", shape="Source Water")
SI_fig12a
ggsave(SI_fig12a, file="Y1_Exp2_tDOMsource/Products/SI_Figures/SI_Figure12a_color.png",device="png", width=5, height=5, units="in", dpi=1200)
...
SHtest_MC<- data2 %>% #create new data frame focused on response
  filter(Treatment=="control" | Treatment =="manure"|Treatment=="crop"|Treatment=="wetland") %>% #treatments selected
  select(Lake, tDOM_added, tDOM_source, Replicate, Microcystin_ugL, pre_Mic)

### write the model ###
MC_ancova<- lm(data=SHtest_MC, 
               log10(Microcystin_ugL) ~ log10(pre_Mic) + tDOM_source) 
...
SI_fig13a<-ggplot(data=SHtest_MC, #plots the model
       aes(x=log10(pre_Mic), y=log10(Microcystin_ugL), color=tDOM_source)) +
  geom_point(aes(shape=Lake)) + 
  geom_smooth(method="lm", se=TRUE) +
 # scale_color_manual(values=c("skyblue3","saddlebrown")) +
  theme_classic() +
  labs(x="Pre Microcystin (ug/L), log10-transformed",
       ...
       color="Treatment")
SI_fig13a
ggsave(SI_fig13a, file="Y1_Exp2_tDOMsource/Products/SI_Figures/SI_Figure13a_color.png",device="png", width=5, height=5, units="in", dpi=1200)
...
SHtest2_Chl<- data2 %>% #create new data frame focused on response
  filter(Treatment!="control")%>% #remove groups already compared 
  select(Lake, Replicate, tDOM_added, tDOM_source, Chla_ugL, pre_Chl) #select fields needed
  
SHtest2_Chl$tDOM_added<-factor(SHtest2_Chl$tDOM_added,
                                   levels=c('none','tDOM added')) #redefine the levels

### write the model ###
Chl_ancova2<- lm(data=SHtest2_Chl, 
               log10(Chla_ugL)~log10(pre_Chl)+tDOM_added*tDOM_source) 
